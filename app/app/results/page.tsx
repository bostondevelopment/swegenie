...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  const { a } = await searchParams;
  const ogUrl = `/api/og?name=${encodeURIComponent(top.name)}&pct=${pct}&d1=${encodeURIComponent(
    dimensionById.get(top.topContributors[0]?.dimension ?? "")?.name ?? ""
  )}&d2=${encodeURIComponent(
    dimensionById.get(top.topContributors[1]?.dimension ?? "")?.name ?? ""
  )}&d3=${encodeURIComponent(dimensionById.get(top.topContributors[2]?.dimension ?? "")?.name ?? "")}`;

  return {
    title: `Top match: ${top.name} — ${pct}% fit`,
    description: `Ranked #1 of ${ranked.length} engineering role archetypes.`,
    openGraph: {
      title: `Top match: ${top.name} — ${pct}% fit`,
      description: `Ranked #1 of ${ranked.length} engineering role archetypes on Roleprint.`,
      images: [{ url: ogUrl, width: 1200, height: 630 }],
    },
    other: { "result-encoded": a ?? "" },
  };
}
